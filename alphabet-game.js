...
// Patch showHome pour bien cacher les écrans OTAN
(function patchShowHomeForOtan() {
  function tryPatch() {
    ...
    window.showHome = function() {
      ...
      ['otanGameScreen', 'otanGamePlayScreen'].forEach(id => {
...
// Injecte le bouton "Alphabet OTAN" sur la home
(function injectOtanButton() {
  function tryInject() {
    const home = document.getElementById('homeScreen');
    if (!home) { setTimeout(tryInject, 200); return; }

    if (home.querySelector('.otan-game-card')) return;

    const heading = document.createElement('div');
    heading.className = 'section-heading';
    heading.innerHTML = 'Alphabet OTAN <span style="color: var(--text-faint); font-weight: 400; font-size: 0.85rem; letter-spacing: 0;">· Entraînement actif par saisie</span>';

    const group = document.createElement('div');
    group.className = 'modes';
    group.innerHTML = `
      <button class="mode-card otan-game-card" onclick="showOtanGame()" style="border: 2px solid var(--gold); position: relative; overflow: hidden; background: linear-gradient(135deg, rgba(217,185,140,0.06), transparent);">
        <div class="mode-number" style="color: var(--gold); font-weight: 700;">🎯 OTAN</div>
        <div class="mode-title">Apprendre en tapant<br>· 4 modes</div>
        <div class="mode-desc">Compléter l'alphabet, mode aléatoire, chrono 60s, ou épeler ton nom SAMY BACHOUCHE. Tu tapes, pas du QCM. Mémorisation active.</div>
        <div class="mode-meta" style="color: var(--gold);">Lancer →</div>
      </button>
    `;

    // Insertion : après "Pratique" (scenarios) si elle existe, sinon avant "Examen Officiel" ou Bonus
    const pratHeading = Array.from(home.querySelectorAll('.section-heading')).find(h => h.textContent.includes('Pratique'));
    const examHeading = Array.from(home.querySelectorAll('.section-heading')).find(h => h.textContent.includes('Examen Officiel'));
    const bonusHeading = Array.from(home.querySelectorAll('.section-heading')).find(h => h.textContent.includes('Bonus'));

    let target = examHeading || bonusHeading;
    if (target) {
      ...
      lastGroup.parentNode.insertBefore(group, heading.nextSibling);
    }
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => setTimeout(tryInject, 200));
  } else {
